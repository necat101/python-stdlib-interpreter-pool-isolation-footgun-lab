...
**Python tested:** 3.12.3 (CPython, Linux) – `InterpreterPoolExecutor` and `concurrent.interpreters` are **not available** in this interpreter. Interpreter-dependent rows are honestly marked `version_skip`.
...
- Rows: 100
- Classifications: pass 15, local_observation 1, version_skip 23, context_only 20, not_applicable 41, fail 0
- Thread-pool: 11 pass, 1 local_observation
- Interpreter-pool: version_skip (API unavailable)
- Preprocessing token-count: serial == thread_pool
...
Evidence was captured before the sentiment summary was prepared. See `hn_comments_sanitized.json` and `hn_thread_evidence.md`.
